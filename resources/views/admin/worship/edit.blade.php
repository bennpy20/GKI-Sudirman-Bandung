...
<form action="{{ route('admin.worship.update', $worship->id) }}" method="POST"
    x-data="{
        ...
        members: @js($members),
        selections: @js($steward_members),
        selectedCategory: '{{ old('category', $worship->category) }}',
        search: {},
        isOpen: {},
            ...
            <div class="p-6 md:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div x-data="{
                            search: '{{ optional($preachers->firstWhere("id", old("preachers_id", $worship->preachers_id)))->name }}',
                            isOpen: false,
                            selectedId: '{{ old('preachers_id', $worship->preachers_id) }}',
                    ...
                    <div>
                        ...
                        <input type="text" name="bible_verse" value="{{ old('bible_verse', $worship->bible_verse) }}" required placeholder="Tuliskan bacaan Alkitab.." class="w-full px-5 py-3 bg-gray-50/50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-church-gold focus:border-church-gold outline-none transition-all text-church-dark font-medium">
                    </div>
                    @error('bible_verse')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="date" value="{{ old('date', $worship->date) }}" required class="w-full px-5 py-3 bg-gray-50/50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-church-gold focus:border-church-gold outline-none transition-all text-church-dark font-medium">
                        </div>
                        @error('date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                        <div>
                            ...
                            <div class="relative">
                                <input type="time" name="time" value="{{ old('time', $worship->time) }}" required class="w-full px-5 py-3 bg-gray-50/50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-church-gold focus:border-church-gold outline-none transition-all text-church-dark font-medium">
                            </div>
                        </div>
                        @error('time')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="video_url" value="{{ old('video_url', $worship->video_url) }}" placeholder="Masukkan tautan.." class="w-full px-5 py-3 bg-gray-50/50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-church-gold focus:border-church-gold outline-none transition-all text-church-dark font-medium">
                    </div>
                    @error('video_url')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                        ...
                        @else
                            <select name="liturgical_calendars_id" class="w-full px-5 py-3 bg-gray-50/50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-church-gold focus:border-church-gold outline-none transition-all text-church-dark appearance-none font-medium">
                                ...
                                @foreach ($liturgical_calendars as $liturgical_calendar)
                                    <option value="{{ $liturgical_calendar->id }}" {{ old('liturgical_calendars_id', $worship->liturgical_calendars_id) == $liturgical_calendar->id ? 'selected' : '' }}>
                                        {{ $liturgical_calendar->name }}
                                    </option>
                                @endforeach
                            </select>
                        @endif
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="5" placeholder="Tuliskan deskripsi tambahan.." class="w-full px-5 py-4 bg-gray-50/50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-church-gold focus:border-church-gold focus:bg-white outline-none transition-all text-church-dark font-medium leading-relaxed resize-y">{{ old('description', $worship->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
